...
int16_t ethernetif_low_input(uint8_t *payload,uint16_t len)
{
  ...
  {
    printf("LwIP pool is full or buffer is too small. Dropping an Ethernet RX frame of size %lu.\n", n);
  }
  else
  {
  ...
  /* Clear Segment_Count */
  if (EthHandle.RxFrameInfos.SegCount != 1)
  {
   printf("Error multiple segments %lu\n", EthHandle.RxFrameInfos.SegCount);
   while(1);
  }
